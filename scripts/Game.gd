...
func _spawn_level():
	...
	while x < target_distance - 200.0:
		var progress: float = clamp(x / target_distance, 0.0, 1.0)
		...
		var step_min: float = max(180.0, obstacle_spacing.x - 20.0 * progress)
		var step_max: float = obstacle_spacing.y
		x += rng.randf_range(step_min, step_max)

	# Extra pickups near the end as a finale
	var px: float = max(520.0, x - 400.0)
...
func _resize_ground() -> void:
	...
	if col and col.shape and col.shape is RectangleShape2D:
		var width: float = max(target_distance + 800.0, 4000.0)
